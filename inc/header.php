<a href="#gnb" class="skip-nav sr-only sr-only-focusable">주메뉴로 건너뛰기</a>
<a href="#content" class="skip-nav sr-only sr-only-focusable">컨텐츠로 건너뛰기</a>
<header id="header">
    <div class="top-menu visible-lg">
        <ul>
            <li>
                <a href="#">HOME</a>
            </li>
            <li>
                <a href="#">SITEMAP</a>
            </li>
            <li>
                <a href="#">CONTACT US</a>
            </li>
            <li class="active">
                <a href="#">협력업체시스템</a>
            </li>
        </ul>
    </div>
    <nav id="gnb" tabindex="-1">
        <div class="wrapper">
            <div class="container">
                <a href="/">
                    <p class="visible-xs main-logo">
                        <img src="/assets/images/main/md_logo.jpg" alt="금도건설 로고">
                    </p>
                    <p class="visible-sm main-logo">
                        <img src="/assets/images/main/md_logo.jpg" alt="금도건설 로고">
                    </p>
                    <p class="visible-md main-logo">
                        <img src="/assets/images/main/md_logo.jpg" alt="금도건설 로고">
                    </p>
                    <p class="visible-lg main-logo">
                        <img src="/assets/images/main/logo.jpg" alt="금도건설 로고">
                    </p>
                </a>
                <div class="hidden-lg md-menu">
                    <button type="button" class="btn btn-menu" aria-hidden="true">
                        <span class="bar"></span>
                        <span class="sr-only">메뉴열기</span>
                    </button>
                </div>
                <ul class="menu-list">
                    <li>
                        <a href="#">회사소개</a>
                        <ul>
                            <li><a href="#">회사소개</a></li>
                            <li><a href="#">CEO인사말</a></li>
                            <li><a href="#">회사연혁</a></li>
                            <li><a href="#">운영조직</a></li>
                            <li><a href="#">찾아오시는길</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#">사업분야</a>
                        <ul>
                            <li><a href="#">사업영역</a></li>
                            <li><a href="#">토목사업</a></li>
                            <li><a href="#">건축사업</a></li>
                            <li><a href="#">민자사업</a></li>
                            <li><a href="#">주택사업</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#">홍보센터</a>
                        <ul>
                            <li><a href="#">CI소개</a></li>
                            <li><a href="#">R&amp;D센터</a></li>
                            <li><a href="#">인증 및 상호</a></li>
                            <li><a href="#">관계사 현황</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#">채용정보</a>
                        <ul>
                            <li><a href="#">인재상</a></li>
                            <li><a href="#">복리후생</a></li>
                            <li><a href="#">채용정보</a></li>
                            <li><a href="#">이력서등록</a></li>
                        </ul>
                    </li>
                    <li>
                        <a href="#">고객센터</a>
                        <ul>
                            <li><a href="#">공지사항</a></li>
                            <li><a href="#">사내소식</a></li>
                            <li><a href="#">건설뉴스</a></li>
                            <li><a href="#">보도자료</a></li>
                            <li><a href="#">고객문의</a></li>
                        </ul>
                    </li>
                </ul>
            </div>
        </div>
    </nav>
</header>
<!-- mobile Gnb
<nav id="nav" class="collapsed hidden-lg" tabindex="-1">
    <div class="nav-backdrop"></div>
    <div class="nav-container">
        <div class="nav-content">
            <div class="nav-head">
                <h1>
                    <img src="/assets/images/main/md_logo.jpg" alt="금도건설 로고">
                </h1>
                <button class="btn btn-nav-close"><span class="glyphicon glyphicon-remove"></span></button>
            </div>
            <ul class="gnb">
                <li>
                    <a href="#">회사소개</a>
                    <ul>
                        <li><a href="#">회사소개</a></li>
                        <li><a href="#">CEO인사말</a></li>
                        <li><a href="#">회사연혁</a></li>
                        <li><a href="#">운영조직</a></li>
                        <li><a href="#">찾아오시는길</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">사업분야</a>
                    <ul>
                        <li><a href="#">사업영역</a></li>
                        <li><a href="#">토목사업</a></li>
                        <li><a href="#">건축사업</a></li>
                        <li><a href="#">민자사업</a></li>
                        <li><a href="#">주택사업</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">홍보센터</a>
                    <ul>
                        <li><a href="#">CI소개</a></li>
                        <li><a href="#">R&amp;D센터</a></li>
                        <li><a href="#">인증 및 상호</a></li>
                        <li><a href="#">관계사 현황</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">채용정보</a>
                    <ul>
                        <li><a href="#">인재상</a></li>
                        <li><a href="#">복리후생</a></li>
                        <li><a href="#">채용정보</a></li>
                        <li><a href="#">이력서등록</a></li>
                    </ul>
                </li>
                <li>
                    <a href="#">고객센터</a>
                    <ul>
                        <li><a href="#">공지사항</a></li>
                        <li><a href="#">사내소식</a></li>
                        <li><a href="#">건설뉴스</a></li>
                        <li><a href="#">보도자료</a></li>
                        <li><a href="#">고객문의</a></li>
                    </ul>
                </li>
            </ul>
        </div>
    </div>
</nav>
-->
